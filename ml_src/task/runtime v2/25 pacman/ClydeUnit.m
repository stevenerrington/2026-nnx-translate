classdef ClydeUnit < mladapter
    ...
    methods
        function obj = ClydeUnit(varargin)
            obj@mladapter(varargin{:});

            load('graphicdata.mat','clyde_left_graphic','clyde_up_graphic','clyde_right_graphic','clyde_down_graphic','ghost_graphic');
            obj.GraphicID(1) = mgladdmovie(clyde_left_graphic,100);
            obj.GraphicID(2) = mgladdmovie(clyde_up_graphic,100);
            obj.GraphicID(3) = mgladdmovie(clyde_right_graphic,100);
            obj.GraphicID(4) = mgladdmovie(clyde_down_graphic,100);
            ...
            obj.DirectionMap = { 3, [3 2], 2, [1 2], 1, [1 4], 4, [3 4];
                1, [1 4], 4, [3 4], 3, [3 2], 2, [1 2] };
        end
        ...
        function next_position(obj,wall,pacman)
            % speed
            ...
            idx = find(~collided);
            if 5==obj.Direction
                pacman_xydeg = obj.Tracker.CalFun.pix2deg(pacman.Position);
                me_xydeg = obj.Tracker.CalFun.pix2deg(obj.Position);
                r = sqrt(sum((pacman_xydeg-me_xydeg).^2));
                theta = acosd((pacman_xydeg(1)-me_xydeg(1))/r);
                if pacman_xydeg(2)-me_xydeg(2)<0, theta = 360-theta; end
                ...
                available_direction = intersect(idx,obj.DirectionMap{obj.State,new_direction_idx});
                if ~isempty(available_direction), idx = available_direction; end

                obj.Direction = idx(ceil(rand*length(idx)));
            end
