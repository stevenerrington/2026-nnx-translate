% The only difference between this and the LBC_fMRI task is that this one
% pre-creates image stimuli and re-uses them over and over. It saves the
% image loading time, during ITI, and the video memory. Note that what is
% provided to LBC_ImageChanger is MGL object IDs, not image filenames.
...
% The 1st column is MGL object IDs.
% The 2nd column is image positions in visual angles.
% The 3rd column is image durations.
% The 4th column is eventmarkers.
ID = TrialRecord.User.ImageID;  % MGL ID from the userloop
img1.ImageList = { ID([1 3]), [5 5; -5 -5], 2000, 101; ...  % A & C
    '', [], 2000, 100; ...
    ID([2 4]), [5 -5; -5 5], 2000, 102; ...  % B & D
    '', [], 2000, 100; ...
    ID([3 1]), [5 5; -5 -5], 2000, 103; ...  % C & A
    '', [], 2000, 100; ...
    ID([4 2]), [5 -5; -5 5], 2000, 104;      % D & B
    '', [], 2000, 100; };
