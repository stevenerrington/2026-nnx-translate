...
%position = GetMonitorPosition(device)
...
if verLessThan('matlab','8.6')
    ...
    position(:,3:4) = position(:,3:4) * DPI_ratio;
    position(:,1:2) = (position(:,1:2)-1) * DPI_ratio + 1;
    position(:,2) = position(:,2) - screensize(4) * (DPI_ratio-1);
end

% sort the order of the positions to be the same as the DirectX's device order.
