classdef ImageChanger < mladapter
    ...
    methods
        ...
        function init(obj,p)
            ...
            % obj.TimeIdx = 1;  % updated together with bImageChanged
            obj.PrevFrame = NaN;
        end
        function fini(obj,p)
        ...
        function continue_ = analyze(obj,p)
            ...
            if obj.bImageChanged, obj.Time(obj.TimeIdx) = p.LastFlipTime; end
            repetition = (CurrentFrame + [1 0]) / obj.ImageSchedule(end);  % the 1st element counts up at the last frame of one rotation; the 2nd, when the new rotation begins.
            obj.Success = obj.Repetition <= repetition(1);  % stop this adapter with the last frame presented
            continue_ = ~obj.Success;

            ImageIdx = find(mod(CurrentFrame,obj.ImageSchedule(end)) < obj.ImageSchedule,1);
            obj.bImageChanged = obj.PrevImageIdx ~= ImageIdx && repetition(2) < obj.Repetition;
            if obj.bImageChanged
                if ~isnan(obj.PrevImageIdx), mglactivategraphic(obj.GraphicID{obj.PrevImageIdx},false); end
                obj.PrevImageIdx = ImageIdx;
                obj.TimeIdx = floor(repetition(2)) * size(obj.List,1) + ImageIdx;
