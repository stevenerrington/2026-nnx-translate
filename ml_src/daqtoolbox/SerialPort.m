classdef SerialPort < dynamicprops
    ...
    methods
        function obj = SerialPort(DeviceID)
            ...
            if ~any(idx), error('SerialPort:AdaptorNotFound','Failure to find requested communication device: %s.',adaptor); end
            ...
            if ~any(idx), error('SerialPort:DeviceNotFound','Constructors require a device ID, e.g. SerialPort(''COM1'').'); end
        ...
        function write(obj,val)  % putvalue
            for m=1:length(obj)
                ...
                mdqmex(29,obj(m).AdaptorName,obj(m).DeviceID,obj(m).TaskID,0,val);
            end                
        end
        function val = available(obj)
            val = mdqmex(29,obj.AdaptorName,obj.DeviceID,obj.TaskID,1);
        end
        function val = read(obj)  % getvalue
            val = mdqmex(29,obj.AdaptorName,obj.DeviceID,obj.TaskID,2);
        end
        function val = readString(obj)
            val = mdqmex(29,obj.AdaptorName,obj.DeviceID,obj.TaskID,3);
        end
        function val = readStringUntil(obj,ch)
            if ~ischar(ch) || 1~=numel(ch), error('The terminator must be a char.'); end
            val = mdqmex(29,obj.AdaptorName,obj.DeviceID,obj.TaskID,4,ch);
        end
        function val = readln(obj,timeout)  % sec
            if ~exist('timeout','var'), timeout = 10; end
            timeout_timer = tic;
            while ~available(obj) && toc(timeout_timer)<timeout, end
            val = strtrim(mdqmex(29,obj.AdaptorName,obj.DeviceID,obj.TaskID,4,char(10))); %#ok<CHARTEN>
        end
        ...
        function set.BaudRate(obj,val)
            mdqmex(25,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'BaudRate',numchk(obj,val)); %#ok<*MCSUP>
        end
        function val = get.BaudRate(obj)
            val = mdqmex(26,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'BaudRate');
        end
        function set.ByteSize(obj,val)
            mdqmex(25,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'ByteSize',numchk(obj,val));
        end
        function val = get.ByteSize(obj)
            val = mdqmex(26,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'ByteSize');
        end
        function set.StopBits(obj,val)
            mdqmex(25,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'StopBits',validateStringSet(obj,'StopBits',val));
        end
        function val = get.StopBits(obj)
            val = mdqmex(26,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'StopBits');
        end
        function set.Parity(obj,val)
            mdqmex(25,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'Parity',validateStringSet(obj,'Parity',val));
        end
        function val = get.Parity(obj)
            val = mdqmex(26,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'Parity');
        end
        function set.Timeout(obj,val)
            mdqmex(25,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'Timeout',numchk(obj,val));
        end
        function val = get.Timeout(obj)
            val = mdqmex(26,obj.AdaptorName,obj.DeviceID,obj.SubsystemType,obj.TaskID,'Timeout');
        end
