classdef mlconditions < handle
    ...
    methods
        function obj = mlconditions(ConditionsFile)
            obj.MLPath = mlpath(mfilename('fullpath'));
            init(obj);
            if exist('ConditionsFile','var'), load_file(obj,ConditionsFile); end
        end
        function init(obj)
            obj.Conditions = [];
        ...
        function load_file(obj,ConditionsFile,wb)
            if ~exist('ConditionsFile','var') || 2~=exist(ConditionsFile,'file')
                [n,p] = uigetfile({'*.txt', 'Conditions file (*.txt)'; '*.m', 'Userloop function (*.m)'},'Select a Conditions File');
            ...
            a = strtrim(regexp(fileread(ConditionsFile),'[^\n]+\n|[^\n]+$','match'));
            b = ~cellfun(@isempty,a);
            cond = cellfun(@obj.unquote,strtrim(regexp(a(b),'\t+','split')'),'UniformOutput',false);
            % in case that the selected file is a userloop file
            if strncmp(cond{1}{1},'function ',9), obj.Conditions = ConditionsFile; end
            if 1==length(cond) && 2==exist(cond{1}{1},'file'), obj.Conditions = which(cond{1}{1}); end
            ...
            for m=1:ncond
                ...
                a = parse_object(obj,cond{m+1}(~isnan(idx)),false);
                ...
                if isnan(taskobj{m,3}) || taskobj{m,3}<0, error('''Frequency'' must be 0 or a natural number (0 or larger)'); end
                if any(isnan(taskobj{m,4})) || any(taskobj{m,4}<1), error('''Block'' must be natural numbers (1 or larger)'); end
            end
            ...
            unique_stim = all_stim(ia);
            test_file_stimuli(obj,unique_stim);

            obj.UIVars.TotalNumberOfConditions = length(val);
            obj.UIVars.StimulusList = unique_stim;
            obj.UIVars.BlockList = unique([val.Block]);
            obj.UIVars.TotalNumberOfConditionsInThisBlock = hist([val.Block],obj.UIVars.BlockList); %#ok<HIST>
            [obj.UIVars.TimingFiles,~,obj.UIVars.TimingFilesNo] = unique({val.TimingFile}');
            
            obj.Conditions = val;
        end
        
        function val = parse_object(obj,str,file_check)
            if ~exist('file_check','var'), file_check = true; end
            ...
            for m=1:nstr
                switch lower(str{m}(1:3))
                    case {'fix','dot'}  % fix(Xpos,Ypos)
                        tokens = regexp(str{m},'^([a-zA-Z]{3}) *\( *(-? *(?:\.\d+|\d+(\.\d*)?)) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *)','tokens');
                    ...
                    case 'pic'  % pic(filename,Xpos,Ypos[,colorkey]) or pic(filename,Xpos,Ypos,width,height[,colorkey])
                        tokens = regexp(str{m},['^([a-zA-Z]{3}) *\( *''?([^'',]+)''? *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *(?:, *)?(\[ *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *\])?(?: *)?\)' ...
                            '|^([a-zA-Z]{3}) *\( *''?([^'',]+)''? *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *, *(\.\d+|\d+(?:\.\d*)?) *, *(\.\d+|\d+(?:\.\d*)?) *(?:, *)?(\[ *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *\])?(?: *)?\)'],'tokens');
                        if ~isempty(tokens)
                            [p,n] = find_ext(obj,tokens{1}{2},{'.png','.jpg','.jpeg','.bmp','.tif','.tiff','.gif'});
                            ...
                            if length(tokens{1})<6, b = sprintf('PIC: %s',n); else, tokens{1}(5:6) = num2cell(round([tokens{1}{5:6}])); b = sprintf('PIC: %s [%d x %d]',n,tokens{1}{5:6}); end
                    ...
                    case 'mov'  % mov(filename,Xpos,Ypos[,looping])
                        ...
                        if ~isempty(tokens)
                            [p,n] = find_ext(obj,tokens{1}{2},{'.3g2','.3gp','.3gp2','.3gpp','.asf','.wmv','.m4a','.m4v','.mov','.mp4','.avi','.mpg','.mpeg','.gif'});
                    ...
                    case 'crc'  % crc(radius,RGB,fill,Xpos,Ypos)
                        tokens = regexp(str{m},'^([a-zA-Z]{3}) *\( *(\.\d+|\d+(?:\.\d*)?) *, *(\[ *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *\]) *, *(\d+(\.\d*)?) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *\)','tokens');
                        if ~isempty(tokens)
                    ...
                    case 'sqr'  % sqr(size,RGB,fill,Xpos,Ypos)
                        tokens = regexp(str{m},['^([a-zA-Z]{3}) *\( *(\.\d+|\d+(?:\.\d*)?) *, *(\[ *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *\]) *, *(\d+(\.\d*)?) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *\)' ...
                            '|^([a-zA-Z]{3}) *\( *(\[ *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *\]) *, *(\[ *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *,? *(?:\.\d+|\d+(\.\d*)?) *\]) *, *(\d+(\.\d*)?) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *\)'],'tokens');
                        if ~isempty(tokens)
                            tokens{1}(2:end) = str2num(obj,tokens{1}(2:end));
                    ...
                    case 'snd'  % snd(filename) or snd(sin,duration,frequency)
                        tokens = regexp(str{m},'^([a-zA-Z]{3}) *\( *''?([^'',]+)''? *\)|^([a-zA-Z]{3}) *\( *''?(?i:sin)''? *, *(\.\d+|\d+(?:\.\d*)?) *, *(\.\d+|\d+(?:\.\d*)?) *\)','tokens');
                        if ~isempty(tokens)
                            if 2==length(tokens{1})
                                [p,n] = find_ext(obj,tokens{1}{2},{'.wav','.mat','.m4a','.aac','.mp3','.ogg','.mp4','.avi'});
                    ...
                    case 'stm'  % stm(port,datasource[,retriggerable])
                        tokens = regexp(str{m},'^([a-zA-Z]{3}) *\( *(\d+) *, *''?([^'',]+)''? *(?:, *)?(\d+)?(?: *)?\)','tokens');
                        if ~isempty(tokens)
                            [p,n] = find_ext(obj,tokens{1}{3},{'.wav','.mat','.m4a','.aac','.mp3','.ogg','.mp4','.avi'});
                            ...
                            b = sprintf('STM: #%d%s, %s',tokens{1}{2},fi(obj,0<tokens{1}{4},' Retriggerable',''),n);
                        end
                    case 'ttl'  % ttl(port)
                    ...
                    case 'gen'  % gen(function_name) or gen(function_name,Xpos,Ypos)
                        tokens = regexp(str{m},'^([a-zA-Z]{3}) *\( *''?([^'',]+)''? *)|^([a-zA-Z]{3}) *\( *''?([^'',]+)''? *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *, *(-? *(?:\.\d+|\d+(\.\d*)?)) *\)','tokens');
                        if ~isempty(tokens)
            ...
            if file_check, test_file_stimuli(obj,val); end
        end
        function test_file_stimuli(~,a)
            for m=1:length(a)
                tokens = a(m).Attribute;
                switch lower(tokens{1})
                    case 'pic', p = tokens{2}; [~,n] = fileparts(p); try mglimread(p); catch err, error('PIC: %s (%s)',err.message, n); end
                    case 'mov', p = tokens{2}; [~,n] = fileparts(p); try mgldestroygraphic(mgladdmovie(p,0)); catch err, error('MOV: %s (%s)',err.message,n); end
                    case 'snd'
                        if 2==length(tokens)
                            p = tokens{2}; [~,n] = fileparts(p); try y = load_waveform(tokens); if isscalar(y), mgldestroysound(y); end, catch err, error('SND: %s (%s)',err.message,n); end
                        end
                    case 'stm', p = tokens{3}; [~,n] = fileparts(p); try load_waveform({'stm',p}); catch err, error('STM: %s (%s)',err.message,n); end
                    case 'gen', p = tokens{2}; [~,n] = fileparts(p); try get_function_handle(p); catch err, error('GEN: %s (%s)',err.message,n); end
                end
            end
        end
        
        function [p,n] = find_ext(obj,name,ext_list)
            [p,n,e] = fileparts(name); name = [n e];
            if ~isempty(p)
                if filesep~=p(end), p = [p filesep]; end
                name = [p name];
            end
            if ischar(ext_list), ext_list = {ext_list}; end
            if any(strcmpi(e,ext_list)), ext_list = [{''} ext_list]; end
            for ext = ext_list(:)'
                [p,n] = mlsetpath([name ext{1}],{obj.MLPath.ExperimentDirectory,obj.MLPath.BaseDirectory,[obj.MLPath.BaseDirectory 'mgl']});
                if ~isempty(p), [~,n,e] = fileparts(n); n = [n lower(e)]; return, end %#ok<AGROW>
            end
            p = ''; n = '';
        end
